...
class getid3_id3v2
{
	...
	function ParseID3v2Frame(&$parsedFrame, &$ThisFileInfo) {
		...
		$parsedFrame['framenameshort'] = $this->FrameNameShortLookup($parsedFrame['frame_name'], $id3v2_majorversion);
	...
	function FrameNameShortLookup($term, $major_version, $reverse = false) {
		if ($major_version < 2 || $major_version > 4) return false;

		static $frames_reverse = array();
		static $frames_forward = array();

		if (empty($frames_forward)) {
			$frames_forward[2] = array(
				...
				'WCP' => 'copyright_information',
				'WPB' => 'url_publisher',
				'WXX' => 'url_user',
				// Unofficial Frames Seend in the Wild
				'TCP' => 'compilation_flag', #itunes
			);

			$frames_forward[3] = array(
				...
				// Unofficial Frames Seed in the Wild
				// Itunes:
				'TCMP' => 'compilation_flag',
				...
				'XRVA' => 'experimental_rva2', # back ported RVA2
			);

			$frames_forward[4] = $frames_forward[3];
		...
		if ($reverse) {
			if (empty($frames_reverse)) {
				foreach($frames_forward as $id => $arr) {
					$frames_reverse[$id] = array_flip($arr);
				}
			}
			$frames = &$frames_reverse;
		} else {
			$frames = &$frames_forward;
		}

		if (isset($frames[$major_version][$term])) {
			return $frames[$major_version][$term];
		} else {
			return false;
		} 
	}
