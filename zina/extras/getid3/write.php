...
class getid3_writetags
{
	...
	var $overwrite_tags     = false;           // if true will erase existing tag data and write only passed data; if false will merge passed data with existing tag data
	...
	var $skip_errors        = false;          // if frame error, skip frame and write tag anyway (id3v2 only)
			...
			switch ($tagformat) {
				case 'ape':
					...
					if (!getid3_lib::IncludeDependency(GETID3_INCLUDEPATH.'write.apetag.php', __FILE__, false)) {
						$this->errors[] = "No $tagformat Library in WriteTags()";
						return false;
					}
					break;
		...
		foreach ($this->tagformats as $tagformat) {
			...
			switch ($tagformat) {
				...
				case 'id3v2.4':
					...
					$id3v2_writer->merge_existing_data = !$this->overwrite_tags;
					$id3v2_writer->skip_errors = $this->skip_errors;
					...
					if (!empty($id3v2_writer->warnings)) $this->warnings = array_merge($this->warnings, $id3v2_writer->warnings);
			...
			if (!$success) {
				if (empty($this->errors)) {
					$this->errors[] = 'unknown error in WriteTags()';
				}
				return false;
			}
		}
					...
					foreach ($valuearray as $key => $value) {
						if (isset($ID3v2_text_encoding_lookup[$id3v2_majorversion][$this->tag_encoding])) {
							...
							if ($id3v2_majorversion < 4) {
								...
								$tag_data_id3v2[$ID3v2_framename][$key]['data']       = getid3_lib::iconv_fallback($this->tag_encoding, 'UTF-16LE', $value);
								$data = &$tag_data_id3v2[$ID3v2_framename][$key]['data'];
								if (!empty($data)) { $data = chr(255).chr(254).$data; }
