...
class getid3_write_id3v2
{
	...
	var $merge_existing_data         = true;     // if true, merge new data with existing tags; if false, delete old tag data and only write new tags
	...
	var $skip_errors                = false;     // if errors in frame, skip and write tag anyway
			...
			switch ($frame_name) {
				...
				case 'WXXX':
					...
					if (!$this->ID3v2IsValidTextEncoding($source_data_array['encodingid'], $this->majorversion)) {
						...
						// need to rewrite IsValidURL() to handle other encodings
						$this->errors[] = 'Invalid URL in '.$frame_name.(isset($source_data_array['data']) ? ' ('.$source_data_array['data'].')' : '');
					} else {
						$framedata .= chr($source_data_array['encodingid']);
						$framedata .= $source_data_array['description'].getid3_id3v2::TextEncodingTerminatorLookup($source_data_array['encodingid']);
						$framedata .= $source_data_array['data'];
	...
	function GenerateID3v2Tag($noerrorsonly=true) {
		...
		if (is_array($this->tag_data)) {
			foreach ($this->tag_data as $frame_name => $frame_rawinputdata) {
				if (!getid3_id3v2::FrameNameShortLookup($frame_name, $this->majorversion)) {
					#$this->warnings[] = 'Ignoring invalid ID3v2 frame type: "'.$frame_name.'"';
					continue;
				}

				foreach ($frame_rawinputdata as $source_data_array) {
					unset($frame_length);
					unset($frame_flags);
					$frame_data = false;
					...
					if (!empty($this->errors)) {
						if ($this->skip_errors) {
							$this->warnings = array_merge($this->warnings, $this->errors);
							$this->errors = array();
							break;
						} else {
							return false;
	...
	function ID3v2ShortFrameNameLookup($majorversion, $description) {
		$description = str_replace(' ', '_', strtolower(trim($description)));
		return getid3_id3v2::FrameNameShortLookup($description, $majorversion, true);
	}
